...
| [AGENT_INTEGRATION.md](./specs/AGENT_INTEGRATION.md) | 多端工具如何通过 MCP 调用、决策规则如何生效 |
...
4. 决定 v0.1 是 Docker-first，还是 Docker + deb 双闭环。
...
| 知识库 | Docker + deb 决策规则（decision-rules.yaml） | 工具输出包含 `decision_basis` |
...
| 决策点 | 时间 | 影响 |
|--------|------|------|
| v0.1 Docker-first 还是 Docker+deb 双闭环 | v0.0 退出前 | 决定 v0.1 范围 |
