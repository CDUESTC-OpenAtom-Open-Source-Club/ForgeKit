...
│   · MCP Server 主协议  · Skill/Markdown 指南  · CLI 兜底        │
└──────────────────────────────────────────────────────────────┘
                                  │
                                  ▼
┌──────────────────────────────────────────────────────────────┐
...
┌──────────────────────────────────────────────────────────────┐
│  MCP Interface Layer                                           │
│   工具注册 · JSON Schema · 执行路由 · 结构化错误 plan_not_found │
└──────────────────────────────────────────────────────────────┘
                                  │
                                  ▼
┌──────────────────────────────────────────────────────────────┐
│  Capability Layer  (原子能力)                                  │
...
└──────────────────────────────────────────────────────────────┘
                                  ...
                                  ▼
┌──────────────────────────────────────────────────────────────┐
│  Local Tooling :  Docker · dpkg/debhelper · shell              │
└──────────────────────────────────────────────────────────────┘

  ═══ decision_basis + 可解释错误 贯穿全链路（AI → MCP → Capability）═══
```
...
flowchart TD
    ...
    AI --> PLAN
    subgraph PLAN["Packaging Plan — Forge.md<br/>产物规范 · 非运行时代码"]
        P1["目标 / 策略 / 决策依据"]
        P2["风险 / 结果"]
    end
    ...
    IF --> CAP
    subgraph CAP["Capability Layer (原子能力)"]
        INS["inspect_project"]
    ...
    CAP --> SYS
    subgraph SYS["System Adapter + Knowledge Layer"]
        VER["versions.yaml"]
        TPL["templates"]
        RULE["decision-rules.yaml + decisions.yaml"]
    end
    SYS --> LOC
    subgraph LOC["Local Tooling"]
        DOCK["Docker"]
        DPKG["dpkg / debhelper"]
        SH["shell"]
    end
    D["decision_basis + 可解释错误<br/>贯穿全链路"] -.-> AI
    D -.-> IF
    D -.-> CAP
```
