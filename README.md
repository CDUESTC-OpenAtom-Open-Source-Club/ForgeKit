...
| 独立开发者 | 交付 Docker/deb/rpm/APK/IPA/exe | 原子工具、模板、决策依据 |
| Web 开发者 | Web 项目变成移动端可用版本 | Web→移动端交付能力 |
...
v0.1–v1.0 的本质不是"加平台"，而是**把同一份可验证交付契约逐端落地**。每届 MVP 见 [`docs/REQUIREMENTS.md`](./docs/REQUIREMENTS.md) §8。

| 端类型 | v0.1 | v0.2 | v0.3 | v0.4 | v0.5 | v1.0 |
|--------|------|------|------|------|------|------|
| **服务器端** | ✅ Docker + deb（可选）· 契约闭环验证 | ✅ rpm + 多语言 · 契约可靠性 | ✅ 稳定 | ✅ | ✅ | ✅ 完整验收 |
| **移动端** | ❌ 不做 | ❌ 不做 | ✅ Android APK + iOS IPA · 契约跨端化 | ✅ 验收 | - | ✅ 完整验收 |
| **Web→移动端** | ❌ 不做 | ❌ 不做 | ✅ PWA + Hybrid（并行） | ✅ 验收 | - | ✅ 完整验收 |
| **桌面端** | ❌ 不做 | ❌ 不做 | ❌ 不做 | ❌ 不做 | 🟡 可选（确认需求后） | ✅ 完整验收 |

**关键决策**：
- v0.1：用最便宜表面证明契约成立（最具战略价值）。
- v0.3：移动端与 Web→移动端并行，验证契约的"可组合 / 跨端"假设。
- v0.5：桌面端仅在确认真实需求后实现（避免过早扩展）。
- v1.0：稳定 API + 贡献指南 + 公开案例，并起步 L2（云构建/签名/分发）。
