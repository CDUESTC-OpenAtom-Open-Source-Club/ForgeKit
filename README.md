...
<p>
  ...
  <img alt="Package version" src="https://img.shields.io/badge/package-v0.2.2--rc.1-cb3837?style=for-the-badge&logo=npm&logoColor=white">
...
| Agent 接入层 | 标准 MCP stdio，暴露七个结构化工具 | Node 18/20 CI 覆盖 |
...
| 适配器 | 状态 | 产物 | 验证边界 |
|------|------|------|----------|
| `servers/ubuntu` | `verified` | Docker/OCI 镜像、`.deb` | 自动化契约测试与 Linux Docker smoke |
| `mobile/harmonyos` | `experimental` | `.hap`、`.app` | 已实现输入校验、计划门禁、签名预检和产物检测；尚缺本轮真实 DevEco 与设备证据 |

Android、iOS、PWA、Windows 和 rpm 不属于本 RC 的发布承诺。
...
| [诊断独立标注流程](./docs/DIAGNOSTIC_ANNOTATION_WORKFLOW.md) | 双维护者盲标、冲突裁决、锁定语料和发布门禁 |
...
| [公开问卷证据综述](./docs/validation/QUESTIONNAIRE_SYNTHESIS_2026-07-22.md) | Issue #1-#5 的证据边界、需求信号和产品判断 |
